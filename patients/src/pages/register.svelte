<script>
    ...
    async function register() {
        loading = true;
        try {
           const c = await client.request(registerUser(
                email,
                password,
                {
            ...
            f7.notification.create({
                ...
                text: 'Check your email for your verification link',
                closeTimeout: 15000,
            }).open();
            disabled = true;
           loading = false;
...
<Page name="auth">
    <div class="bg-zinc-900 py-9 flex  w-full items-center justify-center !bg-cover !bg-no-repeat"
         style="
         background: linear-gradient(rgba(0,0,0,0.9), rgba(0,0,0,0.8)), url('/img/bg-image2.webp');
         background-size: 80%;
        ...
        <form on:submit|preventDefault={register}>
            <div class="grid gap-4 mt-6">
                <div class="grid gap-2">
                    <input type="text" bind:value={lastName} id="lastName" class="placeholder:text-base !text-lg" placeholder="Last Name" required />
                </div>
                <div class="grid gap-2">
                    <input type="text" bind:value={firstName} id="firstName" class="placeholder:text-base !text-lg" placeholder="First Name" required />
                </div>
                <div class="grid gap-2">
                ...
                <div class="grid gap-2 relative">
                    ...
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3" on:click={()=> showPassword = !showPassword}>
                        <Icon icon="heroicons:eye" class="{showPassword && password !== ''  ? 'inline-block' : 'hidden'} h-6 w-6 text-black hover:cursor-pointer" />
                        <Icon icon="heroicons:eye-slash" class="{showPassword  ? 'hidden' : 'inline-block'} h-6 w-6 text-black hover:cursor-pointer"/>
                    </div>
                </div>
                <div class="grid gap-2">
                    <button {disabled} type="submit" class="flex items-center justify-center text-base w-full bg-primary-500 text-white font-medium rounded-lg py-4">
                        {#if loading}
