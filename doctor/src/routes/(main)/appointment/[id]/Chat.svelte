<script>
    ...
    import { formatDateExt, generateSVGAvatar, beep } from "$lib/js/utils2.js";
    import { io } from 'socket.io-client';
    import { PB } from '$lib/js/pb.js';



    export let chatPopupOpened = false;
    export let appointment;
    $: userId = $DOCTOR_DATA?.id;
...
<Popup bind:opened={chatPopupOpened} onBackdropClick={() => (chatPopupOpened = false)}>
    <Page>
        <Navbar
          title="Chat"
          transparent
        ...
        >
            <Link slot="right" navbar onClick={() => (chatPopupOpened = false)}> Close</Link>
        </Navbar>
                ...
                {#if message.sender === userId || message.sender === undefined}
                    <li class="max-w-2xl ml-auto flex justify-end gap-x-2 sm:gap-x-4">
                        ...
                        <span class="flex-shrink-0 inline-flex items-center justify-center h-[2.375rem] w-[2.375rem] rounded-full overflow-hidden bg-gray-600">
                            ...
                            <img src={generateSVGAvatar("Dr")} alt="">
												</span>
                    </li>
                {:else}
                    ...
                    <li class="max-w-2xl mr-auto flex justify-start gap-x-2 sm:gap-x-4">
                        <span class="flex-shrink-0 inline-flex items-center justify-center h-[2.375rem] w-[2.375rem] rounded-full overflow-hidden bg-gray-600">
													  <img src={generateSVGAvatar(appointment?.patient?.last_name + " " + appointment?.patient?.first_name)} alt="">
                        </span>
                        <div class="grow  max-w-sm">
                            ...
                            <p class="text-xs text-gray-400 mt-1">
                                Patient • {formatDateExt(message.created)}
                            </p>
                        </div>
                    </li>
                {/if}
